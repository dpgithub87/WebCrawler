...
public class WebPageDownloaderClient : IWebPageDownloaderClient
{
    ...
    public WebPageDownloaderClient(HttpClient httpClient, ILogger<WebPageDownloaderClient> logger)
    {
    ...
    public async Task<string> DownloadPageAsync(Uri targetUri)
    {
        ...
        try
        {
            response = await _retryPolicy.ExecuteAsync(() => _httpClient.GetAsync(targetUri));
            ...
            {
                _logger.LogInformation($"Content in url:{targetUri} has content type:{contentType}. This is non-html. Ignoring!");
                return null;
            }
